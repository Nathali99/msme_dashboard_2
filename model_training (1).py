...
Original file is located at
    https://colab.research.google.com/drive/1J_gKrM5_Rs7l7WEZidXyx3weNJv49o-Q
"""
...
# Debugging codes
print(X2_train.isna().sum().sum())
print(X2_val.isna().sum().sum())
print(X2_test.isna().sum().sum())

print("\n==============================")
print("PHASE 2 FEATURE BUILDER CHECK")
print("==============================")

print("\nCategorical columns used:")
print(phase2_feature_builder.categorical_columns)

print("\nNumber of categorical columns:")
print(len(phase2_feature_builder.categorical_columns))

print("\nContinuous numeric columns used:")
print(phase2_feature_builder.numeric_columns)

print("\nNumber of continuous numeric columns:")
print(len(phase2_feature_builder.numeric_columns))

print("\nSkewed columns log-transformed:")
print(phase2_feature_builder.skewed_columns)

print("\nNumber of skewed columns:")
print(len(phase2_feature_builder.skewed_columns))

print("\nFinal Phase 2 training feature shape:")
print(X2_train.shape)

print("\nFinal Phase 2 training feature columns:")
print(X2_train.columns.tolist())

print("\nFirst 5 rows of Phase 2 training features:")
display(X2_train.head())

print("\n==============================")
print("FACTOR ANALYSIS FIT RESULTS")
print("==============================")

for group_name, spec in FA_SPECS.items():
    print(f"\n\nFactor group: {group_name}")
    print("-" * 60)

    variables = spec["variables"]
    score_columns = spec["score_columns"]
    fa = phase2_feature_builder.factor_models[group_name]

    print("\nOriginal variables used in this factor group:")
    print(variables)
    ...
    loadings_cutoff = loadings.copy()
    loadings_cutoff = loadings_cutoff.map(
        lambda x: "" if abs(x) < 0.50 else round(x, 3)
    )

    print("\nFactor loadings with cutoff >= 0.50:")
    display(loadings_cutoff)
...
def get_scaled_numeric_data_from_builder(builder, df):
    """
    Reconstructs the scaled numeric data used for final factor analysis.
    This matches the preprocessing inside Phase2FactorFeatureBuilder.
    """
    X_raw = builder._make_phase2_raw(df)
    ...
    X_num = (
        X_features
        .reindex(columns=builder.numeric_columns)
        .apply(pd.to_numeric, errors="coerce")
    )

    X_num = builder._clean_numeric_values(X_num)

    X_num_imputed = pd.DataFrame(
        builder.imputer_num.transform(X_num),
        columns=builder.numeric_columns,
        index=df.index,
    )

    for col in builder.skewed_columns:
        if col in X_num_imputed.columns:
            X_num_imputed[col] = np.log1p(X_num_imputed[col].clip(lower=0))

    X_num_scaled = pd.DataFrame(
        builder.scaler.transform(X_num_imputed),
        columns=builder.numeric_columns,
        index=df.index,
    )
...
def parallel_analysis_eigenvalues(X, n_iter=500, percentile=95, random_state=42):
    """
    Creates random datasets with the same shape as X and calculates
    random eigenvalues. Keep real factors whose eigenvalues are larger
    than the random eigenvalues.
    """
    ...
    random_eigenvalues = np.zeros((n_iter, n_cols))

    for i in range(n_iter):
        random_data = rng.normal(size=(n_rows, n_cols))
        random_corr = np.corrcoef(random_data, rowvar=False)

        eigenvalues = np.linalg.eigvalsh(random_corr)
        eigenvalues = np.sort(eigenvalues)[::-1]

        random_eigenvalues[i, :] = eigenvalues

    return np.percentile(random_eigenvalues, percentile, axis=0)


def check_number_of_factors(
    builder,
    train_df,
    loading_cutoff=0.50,
    n_parallel_iter=500,
):
    X_num_scaled = get_scaled_numeric_data_from_builder(builder, train_df)

    summary_rows = {}

    for group_name, spec in FA_SPECS.items():
        variables = spec["variables"]
        current_n_factors = spec["n_factors"]

        X_group = X_num_scaled[variables].copy()

        print("\n" + "=" * 100)
        print(f"FACTOR NUMBER CHECK: {group_name.upper()}")
        print("=" * 100)

        # KMO and Bartlett checks
        try:
            chi_square_value, bartlett_p_value = calculate_bartlett_sphericity(X_group)
            kmo_per_variable, kmo_model = calculate_kmo(X_group)

            print(f"KMO overall: {kmo_model:.3f}")
            print(f"Bartlett p-value: {bartlett_p_value:.5f}")
        except Exception as exc:
            kmo_model = np.nan
            bartlett_p_value = np.nan
            print(f"KMO/Bartlett check failed: {exc}")

        # Eigenvalues from unrotated factor solution
        fa_check = FactorAnalyzer(
            n_factors=len(variables),
            rotation=None,
        )

        fa_check.fit(X_group)

        eigenvalues, _ = fa_check.get_eigenvalues()
        eigenvalues = np.asarray(eigenvalues)

        parallel_95 = parallel_analysis_eigenvalues(
            X_group,
        ...
        kaiser_n_factors = int(np.sum(eigenvalues > 1))
        parallel_n_factors = int(np.sum(eigenvalues > parallel_95))

        eigen_table = pd.DataFrame(
            {
                "factor_number": np.arange(1, len(eigenvalues) + 1),
                "eigenvalue": eigenvalues,
                "kaiser_keep_eigenvalue_gt_1": eigenvalues > 1,
                "parallel_95_random_eigenvalue": parallel_95,
                "parallel_analysis_keep": eigenvalues > parallel_95,
            }
        )

        print(f"\nCurrent number of factors used in FA_SPECS: {current_n_factors}")
        print(f"Number suggested by eigenvalue > 1: {kaiser_n_factors}")
        print(f"Number suggested by parallel analysis: {parallel_n_factors}")

        display(eigen_table.round(3))

        # Scree plot
        plt.figure(figsize=(8, 5))
        plt.plot(eigen_table["factor_number"], eigen_table["eigenvalue"], marker="o", label="Observed eigenvalue")
        plt.plot(eigen_table["factor_number"], eigen_table["parallel_95_random_eigenvalue"], marker="o", label="Parallel analysis 95% random eigenvalue")
        plt.axhline(y=1, linestyle="--", label="Eigenvalue = 1")
        plt.xlabel("Factor number")
        plt.ylabel("Eigenvalue")
        plt.title(f"Scree plot: {group_name}")
        plt.xticks(eigen_table["factor_number"])
        plt.legend()
        plt.show()

        # Fit final factor model using the current number of factors
        fa_final = builder.factor_models[group_name]

        loadings_df = pd.DataFrame(
            fa_final.loadings_,
            index=variables,
            columns=spec["score_columns"],
        )

        loadings_cutoff = loadings_df.copy()

        try:
            loadings_cutoff = loadings_cutoff.map(
                lambda x: "" if abs(x) < loading_cutoff else round(x, 3)
            )
        except AttributeError:
            loadings_cutoff = loadings_cutoff.applymap(
                lambda x: "" if abs(x) < loading_cutoff else round(x, 3)
            )

        print(f"\nFinal rotated loadings with |loading| >= {loading_cutoff}:")
        display(loadings_cutoff)

        summary_rows[group_name] = {
            "current_n_factors": current_n_factors,
            "kaiser_n_factors": kaiser_n_factors,
            "parallel_analysis_n_factors": parallel_n_factors,
            "kmo_overall": kmo_model,
            "bartlett_p_value": bartlett_p_value,
        }

    summary_df = pd.DataFrame(summary_rows).T.reset_index()
    summary_df = summary_df.rename(columns={"index": "factor_group"})

    print("\n" + "=" * 100)
    print("SUMMARY OF FACTOR NUMBER CHECKS")
    print("=" * 100)
...
factor_number_summary = check_number_of_factors(
    builder=phase2_feature_builder,
    train_df=train_df,
    loading_cutoff=0.50,
    n_parallel_iter=500,
)
